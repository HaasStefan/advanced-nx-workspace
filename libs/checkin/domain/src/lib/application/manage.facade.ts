...
//Alternative when not doing NGRX from the beginning:
@Injectable({ providedIn: 'root' })
export class AlternativeManageFacade {
  private readonly _passengerList = new BehaviorSubject<Passenger[]>([]);
  ...
  load(): void {
    this.passengerService
      .load()
      .subscribe((passengers) => this._passengerList.next(passengers));
  }
}
